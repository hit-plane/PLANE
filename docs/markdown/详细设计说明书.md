...
| 撤下的显示项 | 分数、最高分、难度、火力数字全部不再出现在信息带；难度改由主菜单高亮表示（F26 后连窗口标题也不显示） |
...
| 窗口标题 | 固定为「飞机大战」。`PlaneApp` 不再把档位名拼进标题（该临时调整是同日早些时候加的，未进 SRS，已在 SRS 附录 D-28 的"变更前"一列补记）；`refreshMenuInfo` 相应退回为只刷新最快通关记录的 `refreshMenuRecord`，并移除因此空置的 `Difficulty` import |
