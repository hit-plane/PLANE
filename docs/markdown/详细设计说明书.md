...
| | `FIREPOWER_DURATION` | 10.0 秒（预留：当前火力强化不设时限，见 5.9 N8） | Q6 |
...
| N8 | 火力强化不做时限（v1.0.1 新登记） | Q6 规定「双发持续 10 秒」，当前**永久生效**；`GameConfig.FIREPOWER_DURATION` 已定义但无消费方，属「有常量没逻辑」 | 先把上限与弹道做对（P3），时限需要额外的计时字段与 HUD 反馈，超出本轮范围 | 接 HUD 时一并实现：`Player` 加 `firePowerTimer`，到点回落 1 级；届时 `FIREPOWER_DURATION` 才真正被使用（P21） |
...
| P21 | 低 | `GameConfig.FIREPOWER_DURATION` | 常量已定义（Q6 要求双发持续 10 秒）但**尚无消费方**，当前火力强化永久生效 | 在 `GameModelImpl` 中加火力计时，到点恢复单发 | ⏳ 待修（见 5.9 N8） |
...
| 第四批（新增） | **P18、P19（界面绘制与结算界面）**、P20（BOM 统一）、P21（火力时限） | ⏳ 待做 |
...
| F10 道具 | P1 | 4.7、3.3.2、5.6 | `Item`、`dropItem`、`applyItem` | ✅ 已实现（`applyItem` 兜底 P5 已修）；⚠️ 火力时限未做（P21/N8） |
...
| `PlayerTest` | 16 | 初始状态、四边夹紧、单发/双发、冷却、受伤与无敌帧、护盾、治疗上限、火力上限、死亡、`moveTo` |
