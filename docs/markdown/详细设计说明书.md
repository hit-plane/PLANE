...
| | ~~`FIREPOWER_DURATION`~~ | **已删除**：火力强化改成本局永久生效，不再需要时限常量（见 5.9 N8、6.3 P21） | 🆕 需求变更 |
...
| N8 | ~~火力强化不做时限~~（v1.0.1 登记，v1.3 结案） | 曾与 Q6「双发持续 10 秒」不一致（`FIREPOWER_DURATION` 无消费方） | v1.3 需求变更为**本局永久生效**：删掉 `firePowerTimer` 与常量、配置项，不一致随之消失 | 已结案；火力等级只受道具与开局复位影响（见 6.3 P21） |
...
| P21 | 低 | ~~`GameConfig.FIREPOWER_DURATION`~~ | 曾「有常量没逻辑」，与 P3 一起遗留 | 需求变更为**火力强化本局永久生效**：直接删除 `firePowerTimer` 字段、`update` 内的到点回落分支、`FIREPOWER_DURATION` 常量与 `player.firepower.duration` 配置项 | ✅ **已修复**（改为永久生效，`PlayerTest` 新增「永久不回落」「重复拾取永久叠加」两例，全套 180 例通过） |
...
| F10 道具 | P1 | 4.7、3.3.2、5.6 | `Item`、`dropItem`、`applyItem` | ✅ 已实现（`applyItem` 兜底 P5 已修）；火力强化本局永久生效，原「时限未做」随需求变更结案（P21/N8） |
...
| `PlayerTest` | 21 | 初始状态、四边夹紧、单发/多连发、冷却、受伤与无敌帧、护盾、治疗上限、火力上限、火力强化本局永久（不回落、重复拾取永久叠加）、死亡、`moveTo` |
