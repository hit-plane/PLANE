...
| --- | --- | --- | --- |
| `view.PlaneApp` | `start(Stage)` | 创建 900×700 `Scene`、装配三层、绑定输入与焦点、启动帧循环 | ✅ **已实现**（装配 `GameModelImpl`+`GameView`+`GameController`，绑 P 键与失焦暂停） |
...
| T3 | `PLAYING` | 玩家按 P 键 | `status == PLAYING` | `pause()`，清空按键状态 | `PAUSED` | **[源码]** `GameController#togglePause`（键位 `PAUSE_KEY = P`，已按 SRS F12 对齐） |
| T4 | `PAUSED` | 玩家再按 P 键 | `status == PAUSED` | `resume()`，重置帧计时基准 | `PLAYING` | **[源码]** `GameController#resume` |
...
**无死状态、无不可达状态**：`MENU` 可入可出；`PLAYING`/`PAUSED` 经 P 键或焦点变化互通；`GAME_OVER` 可重开或返回菜单；不存在无出口的非终态。`VICTORY` 已删除（SRS 未定义胜利条件）。
...
| SM-2 | 暂停键位 | ✅ **已对齐**：`GameController.PAUSE_KEY = P`（SRS F12）。**drawio 第 1 页仍标 F12，待改图** |
...
| D8 | 暂停键 | P 键（F12 条目） | 图为 F12、代码未绑定 | **`GameController.PAUSE_KEY = P`**（drawio 图待改） | `GameController` | ✅ 代码已对齐，图形待同步 |
...
| F12 暂停/恢复 | P1 | 3.1 T3/T4/T5/T5b | `GameController#togglePause`、`PlaneApp` 焦点监听 | ✅ 已实现（P 键 + 失焦自动暂停） |
...
预期：弹出 900×700、标题 `Plane` 的窗口；按 **P** 可暂停/恢复；窗口失焦自动暂停、获得焦点自动恢复。
