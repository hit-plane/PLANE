...
// 各机型倍率 = 本档 difficulty.<档>.type.weight.mult.<机型>（简单/普通档均为 1.0；
...
- **缺口**：随机性导致难以写稳定断言；建议给 `Random` 注入固定种子后补一个「1000 次掷点分布」的统计测试（列入 P12）。**已补测**（`GameModelImplTest#bomberSpawnWeightFollowsDifficulty`：注入固定种子随机源，反射调用 `randomType(Difficulty)` 抽样比对三档自爆机数量；`DifficultyTest` 另有 `bomberWeightIsCutOnlyOnHard` 覆盖倍率取值）。
